...
export const updateUser = async (req, res) => {
  try {
    ...
    if (user == 1) res.status(200).send({ message: `El usuario ${username} fue ACTUALIZADO con EXITO!` });
    else {
      res.status(400).send({
...
export const addUserRole = async (req, res) => {
  try {
    ...
    if (!id || !roles) return res.send({ message: "[ERROR] Faltan Argumentos" });

    const user = await Users.findByPk(id);
    if (!user) {
      return res.send({ message: "[ERROR] El Usuario no EXISTE" });
    }

    const userRoles = await UserRol.findAll({ raw: true, where: { users_id: id } });
    let whitelist = await Rol.findAll({ raw: true });
